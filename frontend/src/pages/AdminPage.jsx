...
import { getUserStats, getActiveUsers } from '../api/admin'
...
export default function AdminPage() {
  ...
  useEffect(() => {
    Promise.all([getUserStats(), getActiveUsers()])
      .then(([userRes, activeRes]) => {
        const userData = userRes.data?.data
        if (userData) {
          setTotalUsers(userData.totalUsers ?? 0)
          setMonthlyTrend(userData.monthlyTrend ?? [])
        }
  ...
  const chartYear = useMemo(() => {
    const first = monthlyTrend[0]?.month
    if (first) return String(first).slice(0, 4)
  ...
  const lineData = {
    labels: monthlyTrend.map(m => formatMonthLabel(m.month)),
    datasets: [{
      label: '가입자 수',
      ...
      <div className="adm-content">
        ...
        <div className="adm-summary-row">
          <div className="adm-sum-card">
            ...
            <p className="adm-sum-value">
              {loading ? '—' : fmt(totalUsers)}<span className="adm-sum-unit">명</span>
            </p>
            <p className="adm-sum-sub">일반 회원 누적 (관리자 제외)</p>
          </div>
          <div className="adm-sum-card">
            <p className="adm-sum-label">최근 30일 활성 사용자</p>
            <p className="adm-sum-value">
              {loading ? '—' : fmt(activeUsers)}<span className="adm-sum-unit">명</span>
            </p>
            <p className="adm-sum-sub">최근 30일 내 로그인 (관리자 제외)</p>
          </div>
          <div className="adm-sum-card">
        ...
        <div className="adm-card adm-chart-card">
          <p className="adm-card-title">전체 가입 회원 수 월별 추이 ({chartYear})</p>
          <div className="adm-chart-wrap">
            {loading ? (
              <p className="adm-chart-empty">불러오는 중…</p>
            ) : monthlyTrend.length === 0 ? (
              <p className="adm-chart-empty">가입 데이터가 없습니다.</p>
            ) : (
